...
(function() {
	...
	function walkOn() {
		...
		walker.on('file', function(root, stat, next) {
			var fn = stat.name;
			if (fn.endsWith('.less') && root.indexOf('_') < 0) { // ignore _folders
				if (fn.startsWith('_')) {
	...
	function copyImportsFiles(path) {
		if (fs.lstatSync(path).isDirectory()) {
			...
			dirWalker.on('file', function(root, stat, next) {
				var fn = stat.name;
				if (fn.endsWith('.less') && fn.startsWith('_')) {
					var fp = root + '/' + fn;
    		...
    		zipEntries.forEach(function(zipEntry) {
		        ...
		        if (fn.startsWith('_') && fn.endsWith('.less')) {
	...
	function compile(path, callback) {
		fs.readFile(path, 'utf-8', function (err, data) {
			...
			less.render(data).then(
				function (output) {
					if (callback)
						callback(helper.decodeDsp(output.css), path);
				},
				function (err) {
					removeGenDir();
					throw err;
