...
const fakePropertyRe = new RegExp(FAKE_PROPERTY, "g");
...
const taglibRe = new RegExp('<%.*%>|</?c:.*>|</?z:.*>|</?t:.*>', 'g');
...
module.exports = {
	...
	encodeDsp: function(data, theme, importpath) {
		...
		if (data) {
			...
			for (var i = 0; i < dataArray.length; i++) {
				...
				dataArray[i] = dataArray[i].replace(/__LESSOPEN__/g, '@\{').replace(/__LESSEND__/g, '\}');
	...
	decodeDsp: function(data) {
		...
		return data.replace(/\/\*__TAGLIB /g, '')
				   	  .replace(/ TAGLIB__\*\//g, '')
				      //2. restore EL function like ${c:encodeThemeURL}
				      .replace(/__EL__/g, '${').replace(/__ELSP__/g, ':')
				      .replace(/__ELEND__/g, '}').replace(fakePropertyRe, '');
	},
