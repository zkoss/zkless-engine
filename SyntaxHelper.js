...
module.exports = {
	...
	getMatchedArray: function(str, regexp) {
		var matchArray = [],
			matchedStr = '';

		do {
			matchedStr = str.match(regexp);
			if (matchedStr) {
				var sub = matchedStr[0];
				str = str.substring(str.indexOf(sub) + sub.length);
				matchArray.push(matchedStr[0]);
			}
		} while (matchedStr);
			...
			for (var i = 0; i < dataArray.length; i++) {
				...
				if (current.match(escapeRe)) {      
					...
					var newline = '',
						openRe = new RegExp("@\\{"),
						closeRe = new RegExp("\\}"),
						matchArray = this.getMatchedArray(current, escapeRe);
				...
				if (current.match(elurlRe)) {    
					...
					var newline = '',
						...
						spRe = new RegExp(":"),
						endRe = new RegExp("\\}");
						matchArray = this.getMatchedArray(current, elurlRe);
				...
				if (current.match(elfunRe)) {
					...
					var newline = '',
						...
						spRe = new RegExp(":"),
						endRe = new RegExp("\\}"),
						matchArray = this.getMatchedArray(current, elfunRe);
				...
				if (current.match(taglibRe)) {
					...
					var newline = '',
					matchArray = this.getMatchedArray(current, taglibRe);
